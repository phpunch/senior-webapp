...
def get_args():
    ...
    parser.add_argument("--num-pdfs", type=int, default=-1,
                        help="Num pdfs")
    parser.add_argument("--accepted-overlap", type=float, default=0.2,
                        help="Shows the amount of overlap which is allowed in segment generation from an utterance "
                             "and its corresponding augmented ones.")

    # Parameters for the optimization
    parser.add_argument("--minibatch-size", type=int, dest='minibatch_size', default=128,
                        help="Size of the minibatch used in DNN training.")
...
def process_args(args):
    ...
    if 1 < args.accepted_overlap < 0:
        raise Exception("--accepted-overlap is invalid, it should be between 0 and 1")
...
def get_random_utt_without_replacement(spkr, spk2utt, spk2utt_copy):
    this_utts = spk2utt_copy[spkr]
    this_num_utts = len(this_utts)
    if this_num_utts <= 1:
        spk2utt_copy[spkr] = deepcopy(spk2utt[spkr])
    i = random.randint(0, this_num_utts - 1)
    utt = this_utts.pop(i)
    return utt
...
def get_random_offset(utt_length, length): # เลือก offset
...
def __overlap_is_ok(offset, previous_offsets, length, accepted_overlap):
    for pre_offset, pre_length in previous_offsets:
        diff = abs(pre_offset - offset) * 1.0
        _len = length if offset < pre_offset else pre_length
        if diff / _len < (1 - accepted_overlap):
            return False
    return True


def get_random_offset_with_minimal_overlap(utt_length, length, previous_offsets, accepted_overlap):
    if length > utt_length:
        sys.exit("code error: length > utt-length")
    free_length = utt_length - length
    num_try = utt_length / length + 1
    offset = random.randint(0, free_length)
    while num_try > 0 and not __overlap_is_ok(offset, previous_offsets, length, accepted_overlap):
        offset = random.randint(0, free_length)
        num_try -= 1
    previous_offsets.append((offset, length))
    return offset


def create_list_of_offsets(utt2len):
    utt2offsets = {}
    for key in utt2len.keys():
        idx = key.rfind('-')
        if idx > 0:
            sub_key = key[:idx]
            if sub_key not in utt2len:
                sub_key = key
        else:
            sub_key = key
        if sub_key not in utt2offsets:
            utt2offsets[sub_key] = list()
        utt2offsets[key] = utt2offsets[sub_key]
    return utt2offsets


def kaldi_original_splitting(args, spk2utt, utt2spk, utt2len, prefix):
    ...
    for archive_index in range(args.num_archives):
        ...
        if args.randomize_chunk_length == "true":
            ...
            length = deterministic_chunk_length(archive_index, args.num_archives, args.min_frames_per_chunk,
...
def our_splitting(args, spk2utt, utt2spk, utt2len, prefix):
    # archive_chunk_lengths is an mapping from archive id to the number of
    # frames in examples of that archive.
    archive_chunk_lengths = []
    # all_egs contains 2-tuples of the form (utt-id, offset)
    all_egs = []
    spk2utt_count = {key: max(math.log(len(value)) / math.log(5), 1) for key, value in spk2utt.items()}
    spk2utt_copy = deepcopy(spk2utt)
    utt2offsets = create_list_of_offsets(utt2len)
    info_f = open(args.egs_dir + "/temp/" + prefix + "archive_chunk_lengths", "w")
    if info_f is None:
        sys.exit(str("Error opening file {0}/temp/" + prefix + "archive_chunk_lengths").format(args.egs_dir))
    for archive_index in range(args.num_archives):
        ...
        this_egs = []  # A 2-tuple of the form (utt-id, start-frame)
        spkrs = []
        weight = args.max_frames_per_chunk * 1.0 / length
        for spk in spk2utt.keys():
            spkrs += int(args.num_repeats * weight * spk2utt_count[spk]) * [spk]
        random.shuffle(spkrs)
        for ll in utt2offsets.values():
            del ll[:]
        for n in range(this_num_egs):
            if len(spkrs) == 0:
                print("Ran out of speakers for archive {0}".format(archive_index + 1))
                break
            spkr = spkrs.pop()
            utt = get_random_utt_without_replacement(spkr, spk2utt, spk2utt_copy)
            utt_len = utt2len[utt]
            offset = get_random_offset_with_minimal_overlap(utt_len, length, utt2offsets[utt], args.accepted_overlap)
            this_egs.append((utt, offset))
        all_egs.append(this_egs)
    info_f.close()
...
def our_splitting_per_archive(args, spk2utt, utt2spk, utt2len, prefix):
    # in this case each line is interpreted as follows:
    #  <source-utterance> <absolute-minibatch-index> <relative-minibatch-index> \
    #    <start-frame-index> <num-frames> <speaker-label>
    pdf2num = {}
    spk2utt_count = {key: max(math.log(len(value)) / math.log(5), 1) for key, value in spk2utt.items()}
    spk2utt_copy = deepcopy(spk2utt)
    utt2offsets = create_list_of_offsets(utt2len)
    minibatch_size = args.minibatch_size
    # all_ranges = [''] * args.num_archives
    info_f = open(args.egs_dir + "/temp/" + prefix + "archive_minibatch_count", "w")
    if info_f is None:
        sys.exit(str("Error opening file {0}/temp/" + prefix + "archive_minibatch_count").format(args.egs_dir))
    for archive_index in range(args.num_archives):
        print("Processing archive {0}".format(archive_index + 1))
        this_egs = []
        # spkrs = []
        # for spk in spk2utt.keys():
        #     spkrs += int(args.num_repeats * spk2utt_count[spk]) * [spk]
        spkrs = args.num_repeats * list(spk2utt.keys())
        # shuffle list for creating the minibatches
        random.shuffle(spkrs)
        for ll in utt2offsets.values():
            del ll[:]
        total_generated_frames = 0
        minibatch_index = 0
        while total_generated_frames < args.frames_per_iter:
            if len(spkrs) < minibatch_size:
                print("Ran out of speakers for archive {0}".format(archive_index + 1))
                break
            # select a length for the current minibatch
            if args.randomize_chunk_length == "true":
                length = random_chunk_length(args.min_frames_per_chunk, args.max_frames_per_chunk)
            else:
                length = deterministic_chunk_length(archive_index, args.num_archives, args.min_frames_per_chunk,
                                                    args.max_frames_per_chunk)
            for relative_minibatch_index in range(minibatch_size):
                spkr = spkrs.pop()
                flag = True
                while flag:
                    utterance_index = get_random_utt_without_replacement(spkr, spk2utt, spk2utt_copy)
                    utt_len = utt2len[utterance_index]
                    if utt_len >= length:
                        flag = False
                    else:
                        print("Utterance length {0} is smaller than segment length {1}".format(
                            utt_len, length
                        ))
                offset = get_random_offset_with_minimal_overlap(utt_len, length, utt2offsets[utterance_index],
                                                                args.accepted_overlap)
                this_egs.append((utterance_index, minibatch_index, relative_minibatch_index, offset, length))
                total_generated_frames += length
            minibatch_index += 1
        print("Finished with {0} minibatches count and {1} total generated frames count.".
              format(minibatch_index, total_generated_frames))
        print("{0} {1}".format(archive_index + 1, minibatch_index), file=info_f)
        ranges_file = args.egs_dir + "/temp/" + prefix + "ranges." + str(archive_index + 1)
        f = open(ranges_file, "w")
        if f is None:
            sys.exit("Error opening file " + ranges_file)
        for (utterance_index, minibatch_index, relative_minibatch_index, offset, length) in sorted(this_egs):
            print("{0} {1} {2} {3} {4} {5}".format(utterance_index, minibatch_index, relative_minibatch_index,
                                                   offset, length, utt2spk[utterance_index]), file=f)
            if utt2spk[utterance_index] in pdf2num:
                pdf2num[utt2spk[utterance_index]] += 1
            else:
                pdf2num[utt2spk[utterance_index]] = 1 ### นับจำนวน utt ต่อ spk 1 คน?
        f.close()
        # all_ranges[archive_index] = ranges_file

    # work out how many archives we assign to each job in an equitable way.
    archives_per_job = [list() for _ in range(args.num_jobs)]
    for i in range(args.num_archives):
        archives_per_job[i % args.num_jobs].append(i)

    for job in range(args.num_jobs):
        this_archives_for_job = archives_per_job[job]
        file_path = args.egs_dir + "/temp/" + prefix + "outputs." + str(job + 1)
        fid = open(file_path, "w")
        if fid is None:
            sys.exit("Error opening file " + file_path)
        print("\n".join(["{0}egs.{1}.tar".format(prefix, n + 1) for n in this_archives_for_job]), file=fid)
        fid.close()
...
def main():
    ...
    our_splitting_per_archive(args, spk2utt, utt2spk, utt2len, prefix)
