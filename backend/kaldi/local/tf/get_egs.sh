...
num_train_archives=$[(${num_train_frames}*${num_repeats})/${frames_per_iter} + 1] ### Formula
...
if [ ${stage} -le 2 ]; then
  ...
  for g in $(seq ${num_train_archives}); do
    utils/filter_scp.pl ${temp}/ranges.${g} ${data}/feats.scp > ${temp}/feats.scp.${g} # กรองเอา feat ที่มีเฉพาะใน range เท่านั้น
  done
